...
void detectObjectsCustom(const Mat &img, CascadeClassifier &cascade, vector<Rect> &objects, int scaledWidth, int flags, Size minFeatureSize, float searchScaleFactor, int minNeighbors)
{
    ...
    // Detect objects in the small grayscale image.  //检测人脸
...
void detectLargestObject(const Mat &img, CascadeClassifier &cascade, Rect &largestObject, int scaledWidth)
{
    ...
    int flags = CASCADE_FIND_BIGGEST_OBJECT;// | CASCADE_DO_ROUGH_SEARCH; //只允许查找最大的人脸
    // Smallest object size.
    Size minFeatureSize = Size(80, 80); //检测远离人脸用20*20或者30*30，近端用80*80
    // How detailed should the search be. Must be larger than 1.0.
    float searchScaleFactor = 1.1f;  //设置1.2会检测更快，但是经常找不到人脸
    // How much the detections should be filtered out. This should depend on how bad false detections are to your system.
    // minNeighbors=2 means lots of good+bad detections, and minNeighbors=6 means only good detections are given but some are missed.
    int minNeighbors = 4;  //默认值为3，值越大正确率越高，但是可能会使一些人识别不到
