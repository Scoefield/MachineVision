...
static const char *facerecAlgorithm = "FaceRecognizer.Fisherfaces";
//const char *facerecAlgorithm = "FaceRecognizer.Eigenfaces";
// whereas lower values mean more faces will be classified as "unknown".
static const float UNKNOWN_PERSON_THRESHOLD = 0.7f;

// Cascade Classifier file, used for Face Detection.
static const char *faceCascadeFilename = "/usr/local/opencv3.2.0/share/OpenCV/lbpcascades/lbpcascade_frontalface.xml";     // LBP face detector.
//const char *faceCascadeFilename = "haarcascade_frontalface_alt_tree.xml";  // Haar face detector.
static const char *eyeCascadeFilename1 = "/usr/local/opencv3.2.0/share/OpenCV/haarcascades/haarcascade_lefteye_2splits.xml";   // Best eye detector for open-or-closed eyes.
static const char *eyeCascadeFilename2 = "/usr/local/opencv3.2.0/share/OpenCV/haarcascades/haarcascade_righteye_2splits.xml";   // Best eye detector for open-or-closed eyes.
...
//const char *eyeCascadeFilename1 = "haarcascade_eye_tree_eyeglasses.xml"; // Basic eye detector for open eyes if they might wear glasses.
...
static const int faceWidth = 70;
static const int faceHeight = faceWidth;
...
static const double CHANGE_IN_IMAGE_FOR_COLLECTION = 0.3;      // How much the facial image should change before collecting a new face photo for training.
static const double CHANGE_IN_SECONDS_FOR_COLLECTION = 1.0;       // How much time must pass before collecting a new face photo for training.

static const char *windowName = "WebcamFaceRec";   // Name shown in the GUI window.
static const int BORDER = 8;  // Border between GUI elements to the edge of the image.

static const bool preprocessLeftAndRightSeparately = true;   // Preprocess left & right sides of the face separately, in case there is stronger light on one side.
...
/*-------------------- 函数声明区 -------------------*/
void imageDeal(Mat topLeftOfFace, Mat topRightOfFace, TiredSteerDetectSystemTh *th);
...
void recognizeAndTrainUsingWebcam(VideoCapture &videoCapture, CascadeClassifier &faceCascade, CascadeClassifier &eyeCascade1, CascadeClassifier &eyeCascade2, TiredSteerDetectSystemTh *th)
{
    ...
    while (true) {
        ...
        */
        ...
        QImage img = MatToQImage(displayedFrame);
        emit th->freshMainVideo(img);
        ...
        if (m_debug) {
            ...
            if (faceRect.width > 0) {
                ...
                if (searchedLeftEye.width > 0 && searchedRightEye.width > 0) {
                    ...
                    //imshow("topLeftOfFace", topLeftOfFace);    //对应人的右眼
                    //imshow("topRightOfFace", topRightOfFace);  //对应人的左眼
                    QImage imgLeft = MatToQImage(topLeftOfFace);
                    emit th->freshLeftEyeVideo(imgLeft);

                    QImage imgRight = MatToQImage(topRightOfFace);
                    emit th->freshRightEyeVideo(imgRight);
					//----------------- 眼睛处理算法函数 -----------------
                    imageDeal( topLeftOfFace, topRightOfFace, th);
	...
	/*time0 = ((double)getTickCount()-time0)/getTickFrequency(); //时间测试，大概70多毫秒
	cout << "hello"<<time0<< endl;*/
    }//end while
}
...
void imageDeal(Mat topLeftOfFace, Mat topRightOfFace, TiredSteerDetectSystemTh *th)
{
	Mat element = getStructuringElement( MORPH_ELLIPSE, Size(1,1)); //内核尺寸
	//-------------- 右眼处理 ---------------
	 //眼睛灰度化
	Mat topLeftOfFace_gray;   
	if (topLeftOfFace.channels() == 3) {
		cvtColor(topLeftOfFace, topLeftOfFace_gray, CV_BGR2GRAY);
    }
	else if (topLeftOfFace.channels() == 4) {
		cvtColor(topLeftOfFace, topLeftOfFace_gray, CV_BGRA2GRAY);
    }
	else {
		// Access the input image directly, since it is already grayscale.
			topLeftOfFace_gray = topLeftOfFace;
    }
	//imshow("topLeftOfFace_gray", topLeftOfFace_gray);   

	 //中值滤波
	Mat topLeftOfFace_median;
	medianBlur( topLeftOfFace_gray, topLeftOfFace_median, 5);  //孔径的线性尺寸1/3/5/7/9......
	//imshow("topLeftOfFace_median", topLeftOfFace_median);    

	//直方图均衡化
	Mat topLeftOfFace_high;
	equalizeHist( topLeftOfFace_median, topLeftOfFace_high );

	//二值化
	Mat topLeftOfFace_thresh;
	Mat dstImageLefteye;
	//threshold(topLeftOfFace_median, topLeftOfFace_thresh, 0, 255, CV_THRESH_BINARY|CV_THRESH_OTSU);
	threshold(topLeftOfFace_high, topLeftOfFace_thresh, THRESH, 255, CV_THRESH_BINARY); //二值化
	morphologyEx(topLeftOfFace_thresh, dstImageLefteye, MORPH_CLOSE, element);  //闭运算
	//imshow("topLeftOfFace_thresh", topLeftOfFace_thresh); //显示二值化图像
    //imshow("dstImageLefteye", dstImageLefteye);  //显示闭运算图像
    QImage imgDstImageLefteye = MatToQImage(dstImageLefteye);
    emit th->freshLeftEyeBiVideo(imgDstImageLefteye);  //发信号，刷新UI


	//-------------------- 左眼处理 --------------------
	Mat toprightOfFace_gray;    //灰度化
	if (topRightOfFace.channels() == 3) {
		cvtColor(topRightOfFace, toprightOfFace_gray, CV_BGR2GRAY);
    }
	else if (topRightOfFace.channels() == 4) {
		cvtColor(topRightOfFace, toprightOfFace_gray, CV_BGRA2GRAY);
    }
	else {
		// Access the input image directly, since it is already grayscale.
			toprightOfFace_gray = topRightOfFace;
    }
	//imshow("topLeftOfFace_gray", toprightOfFace_gray);    //眼睛灰度化

    //中值滤波
	Mat toprightOfFace_median;
	medianBlur( toprightOfFace_gray, toprightOfFace_median, 5);  //孔径的线性尺寸1/3/5/7/9......
    //zaas("toprightOfFace_median", toprightOfFace_median);

	//直方图均衡化
	Mat toprightOfFace_high;
	equalizeHist( toprightOfFace_median, toprightOfFace_high );
	//imshow("toprightOfFace_median", toprightOfFace_median);

	//二值化
	Mat toprightOfFace_thresh;
	Mat dstImageRighteye;
	//threshold(toprightOfFace_median, toprightOfFace_thresh, 0, 255, CV_THRESH_BINARY|CV_THRESH_OTSU);//自适应阈值
	threshold(toprightOfFace_high, toprightOfFace_thresh, THRESH, 255, CV_THRESH_BINARY);
	morphologyEx(toprightOfFace_thresh, dstImageRighteye, MORPH_CLOSE, element);  //闭运算
	//imshow("toprightOfFace_thresh", toprightOfFace_thresh);  //显示二值化图像
    //imshow("dstImageRighteye", dstImageRighteye);  //显示闭运算图像
    QImage imgDstImageRighteye = MatToQImage(dstImageRighteye);
    emit th->freshRightEyeBiVideo(imgDstImageRighteye);

	//------------------ 眼睛高度计算函数 -----------------
    if(menuMode == 1)
    eyesHigh( dstImageLefteye, dstImageRighteye, th);

	//---------------------- 眼睛疲劳判断算法函数 ---------------------------
    if(menuMode == 2)
    printmenu_deal( dstImageLefteye, dstImageRighteye, th);

}

int tired_steer_detect_system(TiredSteerDetectSystemTh *th)
{
    //system("color 1F"); //蓝色
    ...
    cout << "Compiled with OpenCV version " << CV_VERSION << endl << endl; //显示Opencv版本

    //double time0 = static_cast<double>(getTickCount());
    // Load the face and 1 or 2 eye detection XML classifiers.
    initDetectors(faceCascade, eyeCascade1, eyeCascade2);  //加载级联分类器
    ...
    */
    ...
    // Get access to the webcam.
    initWebcam(videoCapture, 0);  //打开摄像头

    // Try to set the camera resolution. Note that this only works for some cameras on
    // some computers and only for some drivers, so don't rely on it to work!
    videoCapture.set(CV_CAP_PROP_FRAME_WIDTH, DESIRED_CAMERA_WIDTH);    //640
    videoCapture.set(CV_CAP_PROP_FRAME_HEIGHT, DESIRED_CAMERA_HEIGHT);  //480
    ...
    recognizeAndTrainUsingWebcam(videoCapture, faceCascade, eyeCascade1, eyeCascade2, th);
...
{
    tired_steer_detect_system(this);
}
