...
*/
    ...
    const float EYE_SX = 0.23f;
    const float EYE_SY = 0.35f;
    const float EYE_SW = 0.18f;
    const float EYE_SH = 0.14f;
...
Mat getPreprocessedFace(Mat &srcImg, int desiredFaceWidth, CascadeClassifier &faceCascade, CascadeClassifier &eyeCascade1, CascadeClassifier &eyeCascade2, bool doLeftAndRightSeparately, Rect *storeFaceRect, Point *storeLeftEye, Point *storeRightEye, Rect *searchedLeftEye, Rect *searchedRightEye)
{
    ...
    Rect faceRect;
    detectLargestObject(srcImg, faceCascade, faceRect);  //检测人脸
    ...
    if (faceRect.width > 0) {
        ...
        if (leftEye.x >= 0 && rightEye.x >= 0) {
            ...
            warpAffine(gray, warped, rot_mat, warped.size()); //仿射矩阵
            ...
            bilateralFilter(warped, filtered, 0, 20.0, 2.0);//双边缘滤波
            //imshow("filtered", filtered);

            // Filter out the corners of the face, since we mainly just care about the middle parts.
            // Draw a filled ellipse in the middle of the face-sized image.  椭圆掩码
